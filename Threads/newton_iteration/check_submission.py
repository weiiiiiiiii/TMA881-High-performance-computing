...
def check_runtimes(path):
  return (   check_runtime(1000, 1, 1, path, 0.04, 100)
         ...
         and check_runtime(1000, 1, 5, path, 0.44, 100)
         and check_runtime(1000, 1, 7, path, 0.80, 100)

         and check_runtime(1000, 1, 5, path, 0.44, 100)
         and check_runtime(1000, 2, 5, path, 0.23, 100)
         and check_runtime(1000, 3, 5, path, 0.16, 100)
         and check_runtime(1000, 4, 5, path, 0.12, 100)

         and check_runtime(1000, 10, 7, path, 0.10, 100)
         and check_runtime(2000, 10, 7, path, 0.36, 100)
         and check_runtime(3000, 10, 7, path, 0.80, 100)
         )
...
def run_copy_picture():
  ...
  subprocess.run([extraction_path + "/newton", "-l1000", "-t4", "2"])
  subprocess.run(["convert", "newton_attractors_x2.ppm", "newton_attractors_x2.png"])
  subprocess.run(["convert", "newton_convergence_x2.ppm", "newton_convergence_x2.png"])
  subprocess.run(["mv", "newton_attractors_x2.png", "pictures/" + stem + "_attractors.png"])
  subprocess.run(["mv", "newton_convergence_x2.png", "pictures/" + stem + "_convergence.png"])
  subprocess.run(["rm", "newton_attractors_x2.ppm"])
  subprocess.run(["rm", "newton_convergence_x2.ppm"])
  return True
